								...
								<div class="dataTables_paginate paging_bootstrap">
									<ul class="pagination">
										<li <c:choose><c:when test="${pageNo==1 }">class="prev disabled"</c:when><c:otherwise>class="prev"</c:otherwise></c:choose>>
											<a href="${path }visitorTypeManage?page=${pageNo-1 }&key=${key }"><i class="icon-double-angle-left"></i></a>
										</li>
										<c:forEach var="page" begin="1" end="${pageMax }">
											<li <c:if test="${pageNo==page }">class="active"</c:if>>
												<a href="${path }visitorTypeManage?page=${page }&key=${key }">${page }</a>
											</li>
										</c:forEach>
										<li <c:choose><c:when test="${pageNo==pageMax }">class="next disabled"</c:when><c:otherwise>class="next"</c:otherwise></c:choose>>
											<a href="${path }visitorTypeManage?page=${pageNo+1 }&key=${key }"><i class="icon-double-angle-right"></i></a>
										</li>
									</ul>
								</div>
							</div>
			...
			if(event.keyCode==13){
				...
				var myData = JSON.stringify(visitorType);
			 	$.ajax({  
			        ...
			        url:"${path }visitorTypeManage/save",  
			        data:myData,  
			        dataType: "json",  
			        async: false,  
	...
	/* 删除 */
		$("#table").delegate(".red","click",function(){
			...
			$.ajax({  
		        type: "GET",  
		        contentType:"application/json;charset=utf-8",  
		        url:"${path }visitorTypeManage/del",  
		        data:myData,  
		        dataType: "json",  
		        async: false,  
		        success:function(data){
	...
	/* 恢复 */
		$("#table").delegate("span .green","click",function(){
			...
			$.ajax({  
		        type: "GET",  
		        contentType:"application/json;charset=utf-8",  
		        url:"${path }visitorTypeManage/recover",  
		        data:myData,  
		        dataType: "json",  
		        async: false,  
		        success:function(data){
		...
		$("#table").delegate("#update","keydown",function(event){
			if(event.keyCode==13){
				...
				$.ajax({  
			        type: "POST",  
			        contentType:"application/json;charset=utf-8",  
			        url:"${path }visitorTypeManage/update",  
			        data:myData,  
			        dataType: "json",  
			        async: false,  
			        success:function(data){
